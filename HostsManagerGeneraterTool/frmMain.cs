    ...
    public partial class frmMain : Form
    {
        ...
        private void mnuOpen_Click(object sender, EventArgs e)
        {
            ...
            if (this.openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                src = JsonConvert.DeserializeObject<Source>(sr.ReadToEnd());
        ...
        {
            if(txtInternalName.Text.Length == 0 && txtDestAddress.Text.Length == 0 && txtName.Text.Length == 0 && txtDescription.Text.Length == 0 && txtIntro.Text.Length == 0)
            {
                return;
            }
            if (!txtDestAddress.Text.StartsWith("http://"))
            {
                return;
            }
            if (txtDestAddress.Text.Contains("\n") || txtInternalName.Text.Contains("\n") || txtName.Text.Contains("\n"))
                return;
            if (!Regex.IsMatch(txtInternalName.Text, "[A-Za-z\\.]"))
            {
                return;
            }
            if(!Regex.IsMatch(txtDestAddress.Text, "(http):\\/\\/[\\w\\-_]+(\\.[\\w\\-_]+)+([\\w\\-\\.,@?^=%&amp;:/~\\+#]*[\\w\\-\\@?^=%&amp;/~\\+#])?") || !txtDestAddress.Text.EndsWith(".json"))
            {
                return;
            }
            ...
            saveFileDialog1.FileName = src.internalName;
            if (saveFileDialog1.ShowDialog() == DialogResult.OK && saveFileDialog1.OverwritePrompt)
            {
                StreamWriter sw = new StreamWriter(saveFileDialog1.FileName);
                sw.Write(JsonConvert.SerializeObject(src));
                sw.Close();
            }
        }
